...
import { User, Plus, ArrowRight, ShieldCheck, Wallet, ChevronRight, Lock, KeyRound, HelpCircle, ArrowLeft } from 'lucide-react';
...
const AuthScreen: React.FC<AuthScreenProps> = ({ users, onLogin, onCreateUser, onResetPassword }) => {
  ...
  const handleCreateSubmit = (e: React.FormEvent) => {
    ...
    // Validate password requires security Q&A
                  ...
                  {users.map((user) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="relative flex h-10 w-10 items-center justify-center rounded-full bg-zinc-800 text-zinc-400 group-hover:bg-indigo-500/20 group-hover:text-indigo-400 transition-colors">
                          <User className="h-5 w-5" />
                          {user.password && (
                            <div className="absolute -bottom-1 -right-1 rounded-full bg-zinc-900 p-0.5">
                              <Lock className="h-3 w-3 text-zinc-500 group-hover:text-indigo-400" />
                            </div>
                          )}
                        </div>
          ...
          {view === 'login' && selectedUser && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
               <div className="text-center">
                <h2 className="text-lg font-semibold text-white">Welcome back, {selectedUser.name}</h2>
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                <div className="relative">
                   ...
                   <input 
                     ...
                     className="w-full rounded-lg border border-zinc-700 bg-zinc-800 py-2.5 pl-9 pr-4 text-sm text-white placeholder-zinc-500 focus:border-indigo-500 focus:outline-none"
                ...
                <button
                  type="submit"
                  className="w-full rounded-lg bg-indigo-600 py-2.5 text-sm font-medium text-white shadow-lg shadow-indigo-900/20 transition-all hover:bg-indigo-500 active:scale-95"
                >
                  Unlock Journal
                </button>
              </form>
              ...
              <div className="flex items-center justify-between text-xs">
                 <button onClick={goBack} className="text-zinc-400 hover:text-white flex items-center gap-1">
                   <ArrowLeft className="h-3 w-3" /> Back
                 </button>
                 {selectedUser.securityQuestion && (
                   <button onClick={() => { setError(null); setView('forgot'); }} className="text-indigo-400 hover:text-indigo-300">
                     Forgot Password?
                   </button>
                 )}
              </div>
              ...
              <form onSubmit={handleCreateSubmit} className="space-y-4">
                ...
                <div className="border-t border-zinc-800/50 pt-4">
                   ...
                   <div className="space-y-3">
                      ...
                      {createForm.password && (
                        <div className="animate-in fade-in space-y-3 p-3 rounded-lg bg-zinc-800/50 border border-zinc-800">
                           <div>
                              ...
                              <input
                                ...
                                className="w-full rounded border border-zinc-700 bg-zinc-900 px-2 py-1.5 text-xs text-white focus:border-indigo-500 focus:outline-none"
                              />
                           </div>
                           <div>
          ...
          {view === 'forgot' && selectedUser && (
             <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
               <div className="text-center">
                 ...
                 <p className="text-xs text-zinc-400">Answer your security question to reset</p>
               </div>
               ...
               <div className="rounded-lg bg-zinc-800/50 p-4 border border-zinc-800">
                 ...
                 <p className="text-sm text-zinc-200 font-medium">{selectedUser.securityQuestion}</p>
               </div>
               ...
               <form onSubmit={handleRecoverySubmit} className="space-y-4">
                 <div>
                    <input
                      ...
                      className="w-full rounded-lg border border-zinc-700 bg-zinc-800 py-2.5 px-4 text-sm text-white placeholder-zinc-500 focus:border-indigo-500 focus:outline-none"
                    />
                 </div>
                 {error && <p className="text-xs text-rose-500 text-center">{error}</p>}
                ...
                <button onClick={goBack} className="w-full text-center text-xs text-zinc-400 hover:text-white">Back</button>
             </div>
          )}
